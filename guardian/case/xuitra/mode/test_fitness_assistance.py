...
class TestFitnessAssistance:
    ...
    def leftSwipeMin(self, d, element, percentValue):
        ...
        if d(description="启用实验性功能:").exists():
    ...
    def perpare(self, d):
        ...
        if d(description="取消").exists():
            d.screenshot(rf"E:\Jenkins\SWS_Git\guardian\logs\screen\{timestamp}\DetectAnomalies.jpg")  # 修正路径
    ...
    def test_execution(self):
        ...
        if Flag:
            logging.info("健身模式助力强度设置测试通过")
        else:
            logging.error("健身模式助力强度设置测试异常")
            assert False